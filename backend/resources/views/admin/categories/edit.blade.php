...
@section('content')
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-rose-900 mt-2 dark:text-white"><span data-i18n="admin.categories.edit_title">Edit</span>: {{ $category->name }}</h2>
    </div>
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full px-4 py-2.5 border border-rose-200 rounded-lg text-sm focus:ring-2 focus:ring-rose-500 focus:border-rose-500 outline-none dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100 dark:focus:ring-rose-400 dark:focus:border-rose-400">{{ old('description', $category->description) }}</textarea>
            </div>
            ...
            <div class="pt-2">
                <button type="submit" class="bg-rose-500 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-rose-600 transition shadow-sm shadow-rose-200 dark:shadow-none dark:hover:bg-rose-700" data-i18n="admin.categories.submit_update">
                    Update Category
                </button>
            </div>
        </form>
